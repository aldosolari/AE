...
* la stima delle varianze specifiche, i.e. gli elementi diagonali di $\hat{\Psi}$;

```{r}
af <- factanal(x=X, factors=1, rotation="none") 
af
# stima dei pesi fattoriali
hatLambda_y <- af$loadings[,]
round(hatLambda_y,3)
# veri valori
round(Lambday,3)
...
hatPsi_y <- diag(af$uniquenesses)
round(diag(hatPsi_y),3)
# veri valori
round(diag(Psiy),3)
# differenza vera correlazione e stima
round(Corrx - ( hatLambda_y%*%t(hatLambda_y) + hatPsi_y ),5)
```


4. Calcolare le stime $\hat\Lambda = D^{1/2}\hat \Lambda_z$ e $\hat\Psi = D^{1/2}\hat\Psi_z D^{1/2}$ e confrontare $\hat \Sigma = \hat\Lambda \hat\Lambda' + \hat\Psi$ con $\Sigma$.

```{r}
D = diag(diag(Sigma))
hatLambda = D^(1/2) %*% hatLambda_y 
hatPsi <- D^(1/2) %*% hatPsi_y %*% D^(1/2)
round( hatLambda %*% t(hatLambda) + hatPsi , 3)
Sigma
```
