...
2. Calcolare la matrice di correlazione $\mathbb{C}\mathrm{orr}(x)=D^{-1/2}\Sigma D^{-1/2}$ dove $D = diag(\Sigma)$ e confrontarla con la sua stima $R$ ottenuta dalla matrice dei dati $X$

```{r}
Corrx = diag(diag(Sigma)^(-1/2)) %*% Sigma %*% diag(diag(Sigma)^(-1/2))
round(Corrx,2)
R = cor(X)
round(R,2)
```
...
round(diag(hatPsi),3)
# differenza
R - ( hatLambda%*%t(hatLambda) + hatPsi )
```
