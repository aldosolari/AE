...
( R = cor(X) )
```
Perchè non moltiplichiamo per $((n-1)/n)$? 
...
Per cambiare l'ordine (permutare) dei valori della variabile $x_1$ in maniera causale, si può utilizzare il comando `sample()`. Per riproducibilità dei risultati, impostare all'inizio il seme generatore dei numeri casuali `set.seed(123)`.
...
```{r, message=FALSE}
library(aplpack) # bagplot richiede questo pacchetto
bag <- bagplot(log(Animals))
# per ricavare i valori anomali
bag$pxy.outlier
```
